## Project 1 - Assessing the relationship between US-EU Natural Gas ttfitrage (TTF-Henry Hub) and Energy Select Sector SPDR Fund (XLE) Prices
...
# Calculating weekly change in ttfitrage price
ttf.offset = c(0, t_df_week$c.ttf)
ttf.change <- (t_df_week$c.ttf/head(ttf.offset, -1))-1
t_df_week["ttf.change"] = ttf.change*100
...
rm(df_temp, t_df, temp_list, week_close, ttf.change, ttf.offset, count, i, mmBTU_MWh, USD_EUR, xle.change, xle.offset)
...
t_df_reg2 = t_df_week[-which(t_df_week$ttf.change == max(t_df_week$ttf.change)),]
outlier1 = t_df_week[which(t_df_week$ttf.change == max(t_df_week$ttf.change)),]
outlier2 = t_df_reg2[which(t_df_reg2$ttf.change == max(t_df_reg2$ttf.change)),]
t_df_reg2 = t_df_reg2[-which(t_df_reg2$ttf.change == max(t_df_reg2$ttf.change)),]
...
# Scatterplot
ggplot(data = t_df_reg1, aes(y = xle.change, x = ttf.change)) +
  geom_point() +
  labs(title = "Scatterplot - Weekly Frequency",
       x = "TTF - Henry Hub ttfitrage % Change", y = "XLE Price % Change")
#+ geom_smooth(method='lm', formula= y~x)

# Regression output
lm_1 <- lm( xle.change ~ ttf.change, data=t_df_reg1)
summ(lm_1, digits = 4)
...
# Scatterplot
ggplot(data = t_df_reg2, aes(y = xle.change, x = ttf.change)) +
  geom_point() +
  labs(title = "Scatterplot - Weekly Frequency",
       x = "TTF - Henry Hub ttfitrage % Change", y = "XLE Price % Change")
#+ geom_smooth(method='lm', formula= y~x)
...
# Regression output
lm_2 <- lm( xle.change ~ ttf.change, data=t_df_reg2)
summ(lm_2, digits = 4)
...
# Scatterplot
ggplot(data = t_df_reg3, aes(y = xle.change, x = ttf.change)) +
  geom_point() +
  labs(title = "Scatterplot - Weekly Frequency",
       x = "TTF - Henry Hub ttfitrage % Change", y = "XLE Price % Change")
#+ geom_smooth(method='lm', formula= y~x)
...
# Regression output
lm_3 <- lm( xle.change ~ ttf.change, data=t_df_reg3)
summ(lm_3, digits = 4)
